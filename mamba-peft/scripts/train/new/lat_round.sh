...
for _k in \
  ...
  HP_SPARSE_ENABLED HP_SPARSE_SCOPE HP_SPARSE_BUDGET_MODE \
  HP_SPARSE_FIXED_RATIO HP_SPARSE_FIXED_COUNT HP_SPARSE_MATCH_COUNT \
  HP_SPARSE_MATCH_REFERENCE HP_SPARSE_SCORE_SAMPLES \
  HP_SPARSE_SAMPLE_BATCH_SIZE HP_SPARSE_BASE_INCLUDE HP_SPARSE_BASE_EXCLUDE
do
  v="${!_k-}"
  if [[ -n "${v:-}" ]]; then
    echo "  ${_k}=${v}"
