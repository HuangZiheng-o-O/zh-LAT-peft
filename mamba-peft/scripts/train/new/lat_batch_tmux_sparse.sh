...
# Batch SD-LoRA training script for GLA models.
#
# Tests all 15 SD-LoRA configurations across different datasets.
# Uses lat_round_sparse.sh which directly uses JSON configs via --peft argument.
#
# Usage examples:
#   # Test all SD-LoRA configs on CoLA
#   ./lat_batch_tmux_sparse.sh --round all --pairs "87:glue-tvt_cola"
#
#   # Test on multiple datasets
#   ./lat_batch_tmux_sparse.sh --round all --pairs "87:glue-tvt_cola,87:glue-tvt_mrpc"
#
#   # Test specific rounds (e.g., round 1 = first N configs based on GPU count)
#   ./lat_batch_tmux_sparse.sh --round 1 --pairs "87:glue-tvt_cola" --gpus "0 1 2 3"
#
# Requirements: tmux, awk, nohup. Place this script next to lat_round_sparse.sh.
#

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAUNCHER="${SCRIPT_DIR}/lat_round_sparse.sh"

if [[ ! -x "$LAUNCHER" ]]; then
  echo "ERROR: lat_round_sparse.sh not found or not executable at: $LAUNCHER" >&2
  exit 1
fi
...
MODEL_TYPE="${MODEL_TYPE:-gla}"  # Default to GLA for SD-LoRA
...
print_help() {
  cat <<'EOF'
Usage:
  lat_batch_tmux_sparse.sh --round <N|all> --pairs "SEED:DATA[,SEED:DATA ...]" [options]

Options:
  --round       Round index or 'all' (default: all)
  --pairs       Comma- or space-separated list of seed:data, e.g. "127:glue-tvt_cola,87:glue-tvt_mrpc"
  --model-type  Model type: gla, retnet, mamba2 (default: gla)
  --model       Base model path or HF id (overrides env LAT_MODEL)
  --prec        Precision override (bf16|fp16|fp32)
...
SD-LoRA Configurations (15 total):
  KV configs:     train01, train05, train10, train20, train30 (5)
  QKVO configs:   train01, train05, train10, train20, train30 (5)
  Other configs:  v, vo, qkvog, qkvo_plus_mlp, omlp (all train05) (5)

Example:
  # Test all 15 SD-LoRA configs on CoLA dataset
  ./lat_batch_tmux_sparse.sh --round all --pairs "87:glue-tvt_cola" --gpus "0 1 2 3 4 5 6"

  # Test with specific hyperparameters
  HP_EPOCHS=5 HP_LR=0.0003 HP_BATCH_SIZE=8 \
  ./lat_batch_tmux_sparse.sh --round all --pairs "87:glue-tvt_mrpc"
EOF
}
...
if [[ -z "$PAIRS" ]]; then
  echo "ERROR: --pairs is required (e.g., --pairs \"87:glue-tvt_cola\")" >&2
  exit 2
fi
...
if [[ -z "$SESSION_NAME" ]]; then
  ts="$(date +%m%d_%H%M%S)"
  SESSION_NAME="batch_sdlora_${MODEL_TYPE}_${ROUND}_${ts}"
fi
...
RUNNER="$(mktemp /tmp/sdlora_batch_runner_XXXXXX.sh)"
chmod +x "$RUNNER"
...
echo "== SD-LoRA Batch plan (MODEL_TYPE=${MODEL_TYPE}) =="
...
for item in "${JOBS[@]}"; do
  ...
  data="${item#*:}"
  ts="$(date +%m%d_%H%M%S)"
  sess_step="step${idx}_sdlora_${MODEL_TYPE}_s${seed}_${data}_${ts}"
  log_file="${LOG_DIR}/${sess_step}.log"

  # Prepare temp launcher with FORCE_SEED replaced
  tmp_launcher="$(mktemp /tmp/lat_round_sparse_XXXXXX.sh)"
  awk -v s="$seed" '{
  ...
  (
    ...
    GPU_IDS="$GPU_IDS" GPU_PLAN="$GPU_PLAN" DATA="$data" MODEL_TYPE="$MODEL_TYPE" \
      ...
      bash "$tmp_launcher" "$ROUND" 2>&1 | tee "$log_file"
  )
  status=$?
...
echo "All SD-LoRA jobs finished successfully (MODEL_TYPE=${MODEL_TYPE})."
BODY
} > "$RUNNER"
...
echo ""
echo "SD-LoRA configs (15 total):"
echo "  KV:    train01, train05, train10, train20, train30"
echo "  QKVO:  train01, train05, train10, train20, train30"
echo "  Other: v, vo, qkvog, qkvo_plus_mlp, omlp (all train05)"
echo ""

CMD="start_iso=\$(date +%F_%T); echo \"[\$start_iso] BATCH_CMD: $0 $*\" | tee -a \"$MASTER_LOG\"; bash \"$RUNNER\" | tee -a \"$MASTER_LOG\""

echo "Starting SD-LoRA batch in new tmux session. Your terminal will be attached."
