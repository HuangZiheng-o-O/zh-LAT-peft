...
for item in "${JOBS[@]}"; do
  ...
  (
    ...
    GPU_IDS="$GPU_IDS" GPU_PLAN="$GPU_PLAN" DATA="$data" \
      ...
      HP_EVAL_STEPS="$HP_EVAL_STEPS" HP_SAVE_STEPS="$HP_SAVE_STEPS" HP_LOGGING_STEPS="$HP_LOGGING_STEPS" \
      HP_VAL_SPLIT="$HP_VAL_SPLIT" HP_DATA="$HP_DATA" \
      EVAL_GEN="$EVAL_GEN" EVAL_GEN_MAX_LENGTH="$EVAL_GEN_MAX_LENGTH" \
      EVAL_GEN_MIN_LENGTH="$EVAL_GEN_MIN_LENGTH" EVAL_GEN_NUM_BEAMS="$EVAL_GEN_NUM_BEAMS" \
      GRADIENT_CHECKPOINTING="$GRADIENT_CHECKPOINTING" LOGITS_TO_KEEP="$LOGITS_TO_KEEP" \
