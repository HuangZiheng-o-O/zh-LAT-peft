...
- Processes experiment directories to produce plots, CSVs, and checkpoint summaries.
- Creates a directory for each experiment's output instead of a zip file.
"""
...
def plot_grad_norm(df_train: pd.DataFrame, figdir: Path):
    if "grad_norm" not in df_train.columns or not df_train["grad_norm"].notna().any(): return None
...
def plot_learning_rate(df_train: pd.DataFrame, figdir: Path):
    if "learning_rate" not in df_train.columns or not df_train["learning_rate"].notna().any(): return None
    d = _sorted_by_step(df_train)
    plt.figure()
...
def safe_process_experiment(exp_dir: Path, output_dir: Path):
    ...
    try:
        ...
        if top_1_info:
            (exp_output_dir / "best_checkpoints_top1.txt").write_text(top_1_info, encoding='utf-8')
        if top_3_info:
            (exp_output_dir / "best_checkpoints_top3.txt").write_text(top_3_info, encoding='utf-8')

        print(f"  Successfully created report in: {exp_output_dir}")
        return None

    except Exception as e:
        print(f"  ERROR processing {exp_name}: {e}")
        error_log_path = exp_output_dir / "error_log.txt"
        ...
        print(f"  Created error log at {error_log_path}")
        return f"{exp_name}: {e}"
...
def run_processing(base_dir: str, output_dir: str, workers: int = None):
    ...
    if not experiment_dirs:
        print(f"No valid experiment subdirectories found in {base_dir}")
        return

    print(f"Found {len(experiment_dirs)} experiment dirs in {base_dir} to process using up to {workers or os.cpu_count()} workers.")
    ...
    if errors:
        ...
        for err in errors:
            print(f"- {err}")
    print("\n=== Experiment processing finished! ===")
