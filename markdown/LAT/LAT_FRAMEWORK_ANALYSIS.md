...
UPDATED: 2025-12-30
 GLA SD-LoRA has been elegantly integrated into the existing LAT workflow. Here's a summary of the changes:

  Modified Files

  | File              | Changes                                                                                                                       |
  |-------------------|-------------------------------------------------------------------------------------------------------------------------------|
  | lat_adapter.py    | Added SD-LoRA detection (_detect_peft_type), env overrides (_apply_sdlora_env_overrides), returns 4-tuple with is_sdlora flag |
  | train_lat.py      | Added _run_sdlora_two_phase_training() function for warmup→train phases, integrated into run_train()                          |
  | lat_batch_tmux.sh | Added PEFT env variables (HP_PEFT_TYPE, HP_WARMUP_IT, HP_ZERO_RATIO, etc.)                                                    |
  | lat_round.sh      | Added PEFT env variables to echo list                                                                                         |

  Deleted Files

  | File                | Reason                                             |
  |---------------------|----------------------------------------------------|
  | train_gla_sdlora.py | Redundant - functionality merged into train_lat.py |

  Usage Examples

  # Standard LoRA (default behavior)
  ...
  # SD-LoRA via config file (with peft_type: GLA_SD_LORA)
  python train_lat.py --cfg configs/gla.yaml --peft configs/gla_sdlora/default.json

  # SD-LoRA with custom parameters
  HP_PEFT_TYPE=sdlora HP_WARMUP_IT=200 HP_ZERO_RATIO=0.4 \
    python train_lat.py --cfg configs/gla.yaml

  Workflow

  lat_batch_tmux.sh (HP_PEFT_TYPE=sdlora)
      │
      └── lat_round.sh
              │
              └── train_lat.py
                      │
                      ├── is_sdlora=True → Two-phase training
                      │   ├── Phase 1: Warmup (gradient accumulation)
                      │   └── Phase 2: Training (sparse dimension tuning)
                      │
                      └── is_sdlora=False → Standard LoRA training
